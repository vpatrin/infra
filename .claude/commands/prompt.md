...
2. **General prompt engineering** — craft, review, or advise on prompts for any use case (system prompts, LLM pipelines, CI automation, GitOps, deploy scripts, etc.)
...
- **`command for <description>`** (e.g., `/prompt command for validating Ansible playbooks`) → draft a new slash command file following the established patterns. Present it for approval before writing.
- **Any other input** (e.g., `/prompt system prompt for deploy notifications`, `/prompt query for gitops best practices`, `/prompt review this prompt: ...`) → general prompt engineering mode. Craft, review, or advise on prompts. Apply the same rigor as slash command engineering: clear role, structured output, concrete constraints, no padding.
...
- [ ] Scale calibration present ("single VPS, one developer" — don't over-engineer)
...
|---|---------|---------|---------|-----------|-----|
| 1 | `/qa` | Context gathering | Doesn't read systemd units | Add step to read `*.timer` files | Timers affect backup validation |
